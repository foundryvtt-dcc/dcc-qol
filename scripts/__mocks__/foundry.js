...
const CollectionMock = jest
    ...
    .mockImplementation((entries = []) => {
        const collection = new Map(entries.map((e) => [e._id, e]));

        return {
            get: jest.fn((key) => collection.get(key)),
...
class ActorMock {
    constructor(data, options) {
        ...
        this.items = new global.Collection(data?.items);
        this.prepareData();
...
class ChatMessageMock {
    static getSpeaker = jest.fn(({ scene, actor, token, alias } = {}) => {
        return actor;
    });
...
global.getDCCSkillTableMock = jest
    .fn((skillName) => {
        return null;
    })
    .mockName("game.dcc.getSkillTable");
global.processSpellCheckMock = jest
    .fn((actor, spellData) => {})
    .mockName("game.dcc.processSpellCheck");
global.calculateCritAdjustment = jest
    .fn((original, adjusted) => {
        return 0;
    })
    .mockName("game.dcc.DiceChain.calculateCritAdjustment");
 ...
 * Settings
 */
global.gameSettingsGetMock = jest
    .fn((module, key) => {})
...
global.foundry.utils.expandObject = function (obj, _d = 0) {
    ...
    for (let [k, v] of Object.entries(obj)) {
        if (v instanceof Object && !Array.isArray(v))
            v = global.foundry.utils.expandObject(v, _d + 1);
...
global.foundry.utils.mergeObject = function (
    original,
    other = {},
    {
        insertKeys = true,
    ...
    if (_d === 0 && Object.keys(original).some((k) => /\./.test(k)))
        original = global.foundry.utils.expandObject(original);
    if (_d === 0 && Object.keys(other).some((k) => /\./.test(k)))
        other = global.foundry.utils.expandObject(other);
    ...
    for (let [k, v] of Object.entries(other)) {
        ...
        if (has) {
            ...
            if (tv === "Object" && tx === "Object" && recursive) {
                global.foundry.utils.mergeObject(
                    x,
                    v,
                    {
                        insertKeys,
...
class HooksMock {
    ...
    static on(hook, callback) {}

    static once(hook, callback) {}
}
...
global.rollRenderMock = jest.fn(async (formula) => {
    return "<div>Rendered Roll</div>";
});

global.rollSafeEvalMock = jest.fn((expression) => {
    // ... existing code ...
});
...
class RollMock {
    ...
    constructor(rollData) {
        ...
        // Bind evaluate method to this instance
        this.evaluate = jest.fn(async () => {
            this.total = 8; // Set mock damage total
            return this;
        });
    }
